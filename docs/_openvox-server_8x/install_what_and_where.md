...
| `/opt/puppetlabs/server/bin/` | Private server binaries — not on `PATH` by default |
...
| `ssl/` | SSL certificates, keys, and CRLs managed by the CA service |
...
| Path | Description |
| ---- | ----------- |
| `/var/opt/puppetlabs/puppetserver/` | Server runtime data and CA state |
| `/var/opt/puppetlabs/puppetserver/ssl/` | CA certificates and CRL |
| `/var/run/puppetlabs/puppetserver/` | PID file and socket |
...
- Back up `/etc/puppetlabs/` and `/var/opt/puppetlabs/` before you begin.
